...
#Factorize TransectID and Year later so I can merge climate indices correctly...merge it in the climate compilation file

#remove instances where action is blank, we dont need this
...
txx.flm.5 = felm(ir_tran ~  val.mth + VOR +  Action:val.mth + ObsCode | as.factor(SITE_ID), data = dt.cover.txx.1)

txx.flm.5 = felm(ir_tran ~  val.mth + VOR +  Action:val.mth + ObsCode | as.factor(TransectID), data = dt.cover.txx.1)


txx.flm.6 = felm(ir_tran ~ Action:val.mth + val.mth | as.factor(TransectID), data = dt.cover.txx.1)

txx.flm.6 = felm(ir_tran ~ Action:val.mth + val.mth | as.factor(SITE_ID), data = dt.cover.txx.1)
