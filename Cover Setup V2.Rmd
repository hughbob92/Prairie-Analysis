...
#The formatting is fucked here, read in as excel
data.raw.2 = data.table(readxl::read_xlsx(path = "/Volumes/GoogleDrive/My Drive/Thesis/Prairies/Data And Analysis/GMT Data/Original Data/2019_plot_species_crosstab.xlsx",col_names = T,na = ""))

data.extra.2019 = data.table(read.csv("/Volumes/GoogleDrive/My Drive/Thesis/Prairies/Data And Analysis/GMT Data/Original Data/2019_VOR_Litter.csv",header = T))

mgmt.2019.raw = data.table(read.csv("/Volumes/GoogleDrive/My Drive/Thesis/Prairies/Data And Analysis/GMT Data/Original Data/New_MGMT_Site_MU_20200203.csv",header = T))

```


```{r Data Manipulation}
#The 2019 data format does not match the prior format....this is very annoying and will require a lot of weird matching, here we go

#Merge VOR and Litter data to data.raw.2
data.extra.2019 = data.extra.2019[,.(transect_ID,vor_east,vor_north,vor_south,vor_west,Litter5, Litter10, Litter15, Litter20, Litter25)]

data.2019 = merge(data.raw.2, data.extra.2019, by = "transect_ID")

#2019 data is missing a lot of transect identifying columns, because these are all repeat transects (double check) we can pull this from prior data, except management transects have shifted some. So that data needs to be pulled from the management import
tran.matching = unique(data.raw.1[,.(TransectName,SiteName, SiteID, TransectID,ManUnitID,ManUnitName)])

#Merge in transect identifying info and mgmt unit id
data.2019 = merge(tran.matching, data.2019, by.x = "TransectName", by.y = "transect_ID", all.y = T)

# Missing transects between matching
(data.2019[is.na(TransectID),unique(TransectName)])

###HUGH - turns out this is fucked.

#"1LI1-42" 
data.2019 = data.2019[!TransectName=="1LI1-42",] #only one line for this, remove

#"4LE4-03" & "4LE4-18" are both Shchellburg, which was messy beforehand. According to Daren this was only Protocol A, so drop
data.2019 = data.2019[!TransectName=="4LE4-03",] 
data.2019 = data.2019[!TransectName=="4LE4-18",]


#"3SI5-07" "3SI5-08" "3SI5-09" "3SI5-10" are all Selix_NPB. These were all added in 2019, go ahead and add in and code new TransectIDs for them
data.2019[TransectName=="3SI5-07",":="(TransectID = 8000, SiteName = 'Selix NPB', SiteID = 21, ManUnitID = 79  , ManUnitName = "Selix_South" )] 
data.2019[TransectName=="3SI5-09",":="(TransectID = 8001, SiteName = 'Selix NPB', SiteID = 21, ManUnitID = 79  , ManUnitName = "Selix_South")]
data.2019[TransectName=="3SI5-10",":="(TransectID = 8002, SiteName = 'Selix NPB', SiteID = 21, ManUnitID = 78  , ManUnitName = "Selix_North")]
data.2019[TransectName=="3SI5-08",":="(TransectID = 8003, SiteName = 'Selix NPB', SiteID = 21, ManUnitID = 78  , ManUnitName = "Selix_North")]

#Check if any NAs remain
(data.2019[is.na(TransectID),]) #none

#2019 Data does not use the same species ID info. Match to species ID from prior data and merge in
species.ids = unique(data.raw.1[,.(Scientific, SpeciesID)])
species.ids[,.N, by = Scientific][N>1,] #some scientific names have multiple id's, fixing this in coordination with Plant Reference List
species.ids[SpeciesID==1601,SpeciesID:=330] #Bromus Tectorum
species.ids[SpeciesID==883,SpeciesID:=1597] #Juniperus virginia
species.ids[SpeciesID==1380,SpeciesID:=1602] #Viola pedata
species.ids[SpeciesID==1600,SpeciesID:=100] #Crepis tectorum
species.ids = unique(species.ids)
species.ids[,.N, by = Scientific][N>1,] #fixed

#merge in Species IDs
data.2019 = merge(data.2019, species.ids, by = "Scientific", all.x = T)
unique(data.2019[is.na(SpeciesID),.(Scientific, Common)]) #not all matched - NA, "Aster spp.", "Delphinium carolinianum subsp. virescens", "Elaeagnus angustifolia", "Euphorbia spp.", "juncusspp" 

#fix errors and assign species IDs
data.2019[Scientific=="Aster spp.", SpeciesID:= 3000] #This is tricky because Aster species have been reclassified. - Update species ref list
data.2019[Scientific=="Delphinium carolinianum subsp. virescens", SpeciesID:= 731] #already classified as 731
data.2019[Scientific=="Elaeagnus angustifolia", SpeciesID:= 3001] #new species, invasive and noxious - Update species ref list
data.2019[Scientific=="Euphorbia spp.", SpeciesID:= 3002] #Unknown spurge species - Update species ref list
data.2019[Scientific=="juncusspp" , SpeciesID:= 1616] #already classified as 1616

data.2019[SppCode =="unkncarex1" | SppCode =="unkncarex2", ':=' (SpeciesID = 586, Common = "Sedge species", Scientific = "Carex spp.")] #relabel carex sp appropriately
#The other unknown species never appear consistently at the same transect between monitoring years....HUGH later lump them all together

#Reorder 2019 to be able to merge with prior data
colnames(data.raw.1)

data.cover19 = data.2019[,14:63]
data.cover19[, names(data.cover19):= lapply(.SD, function(x) {x[is.na(x)] <- "" ; x})] #replace NAs with blanks

data.2019 = cbind(data.2019[,.(SiteName,SiteID, ManUnitName, ManUnitID, TransectName, TransectID, primary_observer, secondary_observer, survey_date, Scientific, Common, SpeciesID)],
                  data.cover19,
                  data.2019[,.(vor_north, vor_east, vor_south, vor_west, Litter5, Litter10, Litter15, Litter20, Litter25)])

#Size down original data structure to merge with 2019 data
data.old = data.raw.1[,1:71]

#Correct column names
colnames(data.2019) = colnames(data.old)

#Some columns are in a shitty formmat for 2019, fix - observers, MonDate
data.2019[,PrimObs:= str_replace(PrimObs, "_", " ")]
data.2019[,SecObs:= str_replace(SecObs, "_", " ")]
data.2019[,MonDate:= noquote(format(MonDate,"%m/%d/%y"))]

#Merge two data dumps after subsetting relevant data....HUGH correct when Daren sends VOR and Litter Data
data.raw = rbind(data.old, data.2019)

data.raw[is.na(PrimObs),] #some primary observer data is missing
data.raw[is.na(PrimObs) & is.na(SecObs),] #but nowhere are both missing
data.raw[is.na(PrimObs),PrimObs:=SecObs] #make secondary observer the primary in the absence of primary data 

data.raw[is.na(SecObs),SecObs:=""] #replace NAs with blanks for secondary observer missing data
...
cover.data = data.raw
cover.data = merge(cover.data,obs.code,by.x = "PrimObs",by.y = "UniqueObs")
...
##Necessary subsetting

#Reordering this task
#For some reason two transects were monitored twice in the same year
(unique(cover.data[,length(unique(MonDate)), by = .(TransectID, Year)])[V1>1,]) #Some transects are showing up twice in the same year
cover.data[TransectID==666 & Year==2016,] #Remove the 8/18/16 data
cover.data[TransectID==657 & Year==2018,] #Remove 8/18/18

cover.data = cover.data[!(TransectID==666 & MonDate=="8/18/16"),] 
cover.data = cover.data[!(TransectID==657 & MonDate=="8/18/18"),] 
...
cover.data.long= data.table(melt(cover.data,id=c("TransectID","MonDate","Year","ObsCode","SpeciesID","VOR","Litter"),measure=13:62,variable.name="Plot",value.name="Cover"))
...
#HUGH...revisit this based on discussion with Laura. Important that I do this after subsetting



#Now find species differences
...
#This loop aggregates all unique species IDs for each TransectID from all years. Then it creates a master vector of IDs which it then compares each observed year to in order to create a list of species missing for each year. It outputs vectors which together create a new table specifying the zero cover groups that need to be added
...
#2019 - HUGH run to see what I need to correct
table(cover.data.long[Year=="2019"&Protocol=="C",Cover]) #nothing too zany
...
#No cleaning needed for 2019

###Now creating a binning reference
dat.year = c("2008","2009", "2010", '2011',"2012","2013","2014","2015","2016","2017","2018",'2019')
Prot.C = data.table()
...
key.dat = as.data.table(readxl::read_xlsx("/Volumes/GoogleDrive/My Drive/Thesis/Prairies/Data And Analysis/GMT Data/Bin visualization2.xlsx",sheet = "Bin Key"))

key.dat

#Map methodology to years....HUGH double check this with 2019 data
Start = c()
End = c()
for (i in 2008:2019) {
  Start = c(Start,rep(i,times=(2020-i)))
  End = c(End,c(i:2019))
}
...
method.key = data.table(Start = Start, End = End, Method = 
                          ifelse((Start==2008|Start==2009|Start==2010|Start==2011|Start==2012)&(End==2008|End==2009|End==2010|End==2011|End==2012)|
                                  (Start==2013 & End==2013)|
                                  (Start==2014 & End==2014)|
                                  (Start==2015|Start==2016|Start==2017|Start==2018|Start==2019) & (End==2015|End==2016|End==2017|End==2018|End==2019),
                                  "Method 1",
                                 ifelse((Start==2008|Start==2009|Start==2010|Start==2011|Start==2012) & (End==2013|End==2014|End==2015|End==2016|End==2017|End==2018|End==2019),
                                    'Method 2',
                                    ifelse((Start==2013|Start==2014) & (End==2015|End==2016|End==2017|End==2018|End==2019),
                                           'Method 3', 'Method 4'))))
...
#Now there are multiple datasets after the 2019 update. To the extent possible, load them in separately and then merge them into one file

#First load in all necessary datasets
management.data.raw.1 =data.table(readxl::read_xlsx("/Volumes/GoogleDrive/My Drive/Thesis/Prairies/Data And Analysis/GMT Data/Original Data/MGMT_Action_Site_MU_20190311.xlsx"))
management.data.raw.2 =data.table(readxl::read_xlsx("/Volumes/GoogleDrive/My Drive/Thesis/Prairies/Data And Analysis/GMT Data/Original Data/New_MGMT_Site_MU_20200203.xlsx"))
...
#MGMT
management.data.1 = management.data.raw.1[,.(SiteID,SiteName,ManUnitID,StartDate,EndDate,MgmYr,Action)]
management.data.2 = management.data.raw.2[,.(SITE_ID,SITE, tbl_Mgmtactions_transect_MGMTUNIT,StartDate,EndDate,MgmYr,action)]

colnames(management.data.2) = colnames(management.data.1)

management.data = rbind(management.data.1, management.data.2)

acres = unique(management.data.raw.1[,.(ManUnitID,ManUnitAcres)])

management.data = merge(management.data, acres, by = "ManUnitID",  all = T)

#System Type ---- THIS IS NO LONGER RELEVANT DUE TO FIXED EFFECTS               
...
#This file needs to be in wide format. Originally there are duplicate rows for transect ID because of observation years...HUGH add in 2019 
...
#Since I don't have site info for 2019 I need to bring this in from elsewhere...will use the cover data
Obs.2019 = data.table(Transects = cover.data[Year==2019,unique(TransectID)], Obs.2019 = rep(c(1), times = length(cover.data[Year==2019,unique(TransectID)])))

C.transects.merge.1 = merge(C.transects.merge.1,Obs.2019, by = c("Transects"), all = T)
C.transects.merge.1[, names(C.transects.merge.1):= lapply(.SD, function(x) {x[is.na(x)] <- "0" ; x})] #replace NAs with blanks
...
C.transects.merge.2 = matrix(systems,ncol = 12,byrow = T)
...
#2019 system type info is not available......
...
manage.merge.long.2 = matrix(actions,ncol = length(2008:2019),byrow = T)
colnames(manage.merge.long.2)=c("Mgmt.2008","Mgmt.2009","Mgmt.2010","Mgmt.2011","Mgmt.2012","Mgmt.2013","Mgmt.2014","Mgmt.2015","Mgmt.2016","Mgmt.2017","Mgmt.2018", "Mgmt.2019")
...
#I'm leaving out system type to make this less complex
...
transect.master.1$MGMTUNIT_I=as.numeric(transect.master.1$MGMTUNIT_I) #make same class to match
transect.master.3 = merge(transect.master.1,manage.merge.long.2,by.x = "MGMTUNIT_I",by.y = "UnitIds",all = TRUE)
...
transect.master=transect.master.3[,.(Transect_I,TransectNa,StartLat,StartLong,SITE_ID,tblSITE_Site,MGMTUNIT_I,ManUnitAcres,Obs.2008, Obs.2009, Obs.2010, Obs.2011, Obs.2012, Obs.2013, Obs.2014, Obs.2015, Obs.2016, Obs.2017,Obs.2018, Obs.2019, Mgmt.2008, Mgmt.2009, Mgmt.2010, Mgmt.2011, Mgmt.2012, Mgmt.2013,Mgmt.2014, Mgmt.2015, Mgmt.2016, Mgmt.2017, Mgmt.2018, Mgmt.2019)]

transect.master


#HUGH - get the new transects in here better, but this work for now
```
...
species.dat = species.dat[,.(SpeciesID,Species.Cat,Inclusion.Cat,Indicator,Ind.class,Native.Status,Duration, Type, Legume, Photopath, Range, Distribution, Season)] #add in more as I complete plant trait data
