...
library(data.table)
library(stringr)
data.raw = data.table(read.csv("/Volumes/GoogleDrive/My Drive/Thesis/Prairies/Data And Analysis/GMT Data/Original Data/Plot_protC_Species_50plots_20190313.csv",header = T))
```
...
data.raw[,unique(SecObs)] #''
#obs.com = data.frame(UniqueObs=paste(data.raw[,PrimObs],data.raw[,SecObs]),RowOrder = 1:length(data.raw[,PrimObs])) #concatenate, choosing to only use the primary observer
...
key.dat = as.data.table(readxl::read_xlsx("Bin visualization.xlsx",sheet = "Bin Key"))
...
mgmt.method.1.1 = mgmt.setup.1.1[,(freq=sum(Bin)), by= .(MGMTUNIT_I,Year,MonDate, Protocol, tran.count,SpeciesID)] #can no longer use Observer covariate
...
transect.master ...complete with soil data
...
species.dat = species.dat[,.(SpeciesID,Species.Cat,Inclusion.Cat,Indicator,Native.Status)] #add in more as I complete plant trait data
species.dat$SpeciesID=as.numeric(species.dat$SpeciesID)
cover = merge(cover,species.dat,by = "SpeciesID")
