...
tran.matching = unique(data.raw.1[,.(TransectName,SiteName, SiteID, TransectID)])
mgmt.matching = unique(mgmt.2019.raw[,.(SITE,SITE_ID,tblMGMTUNIT_MGMTUNIT, tbl_Mgmtactions_transect_MGMTUNIT, MgmYr)])

data.2019 = merge(tran.matching, data.2019, by.x = "TransectName", by.y = "transect_ID", all.y = T)

###HUGH - turns out this is fucked. Revisit with new data
...
table(cover.data.long[Year=="2019"&Protocol=="C",Cover])
...
setnames(management.data.2, old = c('SITE_ID','SITE', 'tbl_Mgmtactions_transect_MGMTUNIT','action'), 
         new = c('SiteID','SiteName','ManUnitID','Action'))

management.data = rbind(management.data.1, management.data.2)

acres = unique(management.data.raw.1[,.(ManUnitID,ManUnitAcres)])

management.data = merge(management.data, acres, by = "ManUnitID")
