...
cover[Freq.T>0,':='(f.ind.leg = mean(Freq.T), rel.cov.2.ind.leg = sum(rel.cov.T.2), meanDI.2.ind.leg = mean(DI.T.2)),
      by = .(Year,Legume,TransectID,Ind.class)]

cover[Freq.T>0,':='(f.indlvl.leg = mean(Freq.T), rel.cov.2.indlvl.leg = sum(rel.cov.T.2), meanDI.2.indlvl.leg = mean(DI.T.2)),
      by = .(Year,Legume,TransectID,Indicator)]

cover = merge(cover,cover[Freq.T>0,.(r.T.leg = .N), by=.(Year,Legume,TransectID,Native.Status)], by =c('TransectID','Year','Native.Status','Legume'),all = T)
cover[is.na(r.T.leg),r.T.leg:=0] #Because with these groups there will be years with zero species in the group, to adequate calculate lagged values we need these to be zeros, same goes for below

cover = merge(cover,cover[Freq.T>0,.(r.ind.leg = .N), by=.(Year,Legume,TransectID,Ind.class)], by =c('TransectID','Year','Ind.class','Legume'),all = T)
cover[is.na(r.ind.leg),r.ind.leg:=0] 

cover = merge(cover,cover[Freq.T>0,.(r.indlvl.leg = .N), by=.(Year,Legume,TransectID,Indicator)], by =c('TransectID','Year','Indicator','Legume'),all = T)
cover[is.na(r.indlvl.leg),r.indlvl.leg:=0] 

cover[order(Year),':='(f.T.leg.lag = shift(f.T.leg), rel.cov.2.leg.lag = shift(rel.cov.2.leg), meanDI.2.leg.lag = shift(meanDI.2.leg)),by = .(Legume,TransectID,Native.Status,SpeciesID)]

cover[order(Year),':='(f.ind.leg.lag = shift(f.ind.leg), rel.cov.2.ind.leg.lag = shift(rel.cov.2.ind.leg), meanDI.2.ind.leg.lag = shift(meanDI.2.ind.leg)),by = .(Legume,TransectID,Ind.class,SpeciesID)]

cover[order(Year),':='(f.indlvl.leg.lag = shift(f.indlvl.leg), rel.cov.2.indlvl.leg.lag = shift(rel.cov.2.indlvl.leg), meanDI.2.indlvl.leg.lag = shift(meanDI.2.indlvl.leg)),by = .(Legume,TransectID,Indicator,SpeciesID)]

cover[order(Year),r.T.leg.lag:=shift(r.T.leg),by = .(Legume,SpeciesID,TransectID,Native.Status)]

cover[order(Year),r.ind.leg.lag:=shift(r.ind.leg),by = .(Legume,SpeciesID,TransectID,Ind.class)]

cover[order(Year),r.indlvl.leg.lag:=shift(r.indlvl.leg),by = .(Legume,SpeciesID,TransectID,Indicator)]
