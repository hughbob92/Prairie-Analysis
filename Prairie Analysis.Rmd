...
cover[!(cov.2.T==0), sr_tran:=length(unique(SpeciesID)), by = .(TransectID, Year)]
cover[!(cov.2.T==0), sr_mgmt:=length(unique(SpeciesID)), by = .(MGMTUNIT_I, Year)]
cover[!(cov.2.T==0), sr_site:=length(unique(SpeciesID)), by = .(SITE_ID, Year)]
...
#Compute native and non-native richness by transect, mgmt unit, site for each year
cover[Native.Status=="I" & !(cov.2.T==0), ir_tran:=length(unique(SpeciesID)), by = .(TransectID, Year)]
cover[Native.Status=="I" & !(cov.2.T==0), ir_mgmt:=length(unique(SpeciesID)), by = .(MGMTUNIT_I, Year)]
#cover[Native.Status=="I" & !(cov.2.T==0), ir_site:=length(unique(SpeciesID)), by = .(SITE_ID, Year)]

cover[Native.Status=="N" & !(cov.2.T==0), nr_tran:=length(unique(SpeciesID)), by = .(TransectID, Year)]
cover[Native.Status=="N" & !(cov.2.T==0), nr_mgmt:=length(unique(SpeciesID)), by = .(MGMTUNIT_I, Year)]
#cover[Native.Status=="N" & !(cov.2.T==0), nr_site:=length(unique(SpeciesID)), by = .(SITE_ID, Year)]

#cover[Native.Status=="U" & !(cov.2.T==0), ur_tran:=length(unique(SpeciesID)), by = .(TransectID, Year)]
#cover[Native.Status=="U" & !(cov.2.T==0), ur_mgmt:=length(unique(SpeciesID)), by = .(MGMTUNIT_I, Year)]
#cover[Native.Status=="U" & !(cov.2.T==0), ur_site:=length(unique(SpeciesID)), by = .(SITE_ID, Year)]
...
#Sum up realtive cover by invasive status
...
###Create a mean dominance by native status.....Hugh right now don't break out my methodology
...
#Invasive
cover[!(is.na(DI.T.1.1)) & Native.Status=="I",meanDI.I_tran:=mean(DI.T.1.1), by = .(TransectID,Year)]
cover[!(is.na(DI.T.1.2)) & Native.Status=="I",meanDI.I_tran:=mean(DI.T.1.2), by = .(TransectID,Year)]
cover[!(is.na(DI.T.2)) & Native.Status=="I",meanDI.I_tran:=mean(DI.T.2), by = .(TransectID,Year)]
cover[!(is.na(DI.T.3)) & Native.Status=="I",meanDI.I_tran:=mean(DI.T.3), by = .(TransectID,Year)]
cover[!(is.na(DI.T.4)) & Native.Status=="I",meanDI.I_tran:=mean(DI.T.4), by = .(TransectID,Year)]
